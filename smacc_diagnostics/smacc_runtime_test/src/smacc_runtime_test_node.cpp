...
public:
  ...
  void init()
  {
    ...
    smaccTransitionSub_ = nh_.subscribe(stateMachineROSParamWs + "/smacc/transition_log", 1,
                                        &SmaccTestRuntimeNode::onSmaccTransitionCallback, this);
...
class ReachedStateSuccessTestPolicy : public TestPolicy
{
  ...
  virtual void init(XmlRpc::XmlRpcValue& initXmlRpcValue) override
  {
    ...
    this->owner_->onSmaccTransition.connect([=](auto& msg) {
...
TEST(TestSuite, testCase1)
{
  ...
  while (ros::ok() && !testNode.cleanSuccessExit_)
  {
    testNode.update();
    ros::spinOnce();
    r.sleep();
